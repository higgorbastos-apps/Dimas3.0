...
.topic-done-btns{display:flex;gap:8px;}
.topic-btn-continue{
...
.topic-btn-continue:hover{color:var(--cream);border-color:var(--muted);}
.topic-btn-menu{
  flex:2;padding:10px 14px;
  background:var(--gold);border:none;
  border-radius:8px;color:#1A1304;font-size:14px;
  font-weight:700;cursor:pointer;font-family:var(--sans);
  transition:filter .15s;
}
.topic-btn-menu:hover{filter:brightness(1.08);}
`;
...
${(resumo.maisOcadas||[]).map((m,i)=>`${i+1}. ${m.musica} — ${m.vezes}x (última: ${m.ultimaVez})`).join('\n')}

MÚSICAS ESQUECIDAS (>60 dias sem tocar):
${(resumo.esquecidas||[]).length>0?(resumo.esquecidas||[]).map(m=>`• ${m.musica} (última: ${m.ultimaVez})`).join('\n'):'• Nenhuma.'}

FREQUÊNCIA DE SHOWS:
${(resumo.showsPorMes||[]).map(s=>`• ${s.mes}: ${s.qtd} show${s.qtd>1?'s':''}`).join('\n')}

ÚLTIMOS 5 SHOWS:
${(resumo.ultimos5||[]).map(s=>`• ${s.data} — ${s.local} (${s.qtd} músicas)${s.setlist?.length>0?' | '+s.setlist.slice(0,4).join(', '):''}`).join('\n')}
` : `
...
PERFIL:
• Nome: ${p.nome||"—"} | Formação: ${p.formacao||"—"} | Região: ${p.regiao||"—"}
• Gêneros AUTORIZADOS: ${(p.generos||[]).join(", ")||"—"}
• Referências: ${p.referencias||"—"}
• Tipos de evento: ${(p.tipos_evento||[]).join(", ")||"—"}
...
3. USE APENAS OS GÊNEROS DO PERFIL acima. Jamais inclua estilos não listados.
4. NUNCA invente músicas ou atribuições erradas. Só cite títulos que você tem certeza absoluta que existem e pertencem ao artista correto. Na dúvida, omita.
5. USE O HISTÓRICO: evite repetir músicas tocadas recentemente (últimos 2 shows). Priorize músicas esquecidas para reintroduzir variedade.
6. SEJA AUTÔNOMO: não espere o artista listar músicas. Você é o especialista — monte listas concretas com títulos reais dos gêneros do perfil. O artista contratou você exatamente para isso.

FORMATO DE SETLIST:
Organize em blocos com nome e função. Para cada música: número | título | artista original | duração estimada | motivo da escolha.
Ao final do setlist, informe: total de músicas, duração estimada total, e se há músicas do histórico reintroduzidas.
...
export default function DirectorMusical(){
  ...
  const [topicDone, setTopicDone] = useState(false); // true when Director signals end of topic
...
Extraia apenas informações REALMENTE relevantes sobre o artista (preferências, feedbacks, aprendizados). Se não houver nada relevante, retorne {"memorias":[]}.`
  ...
  const sendMessage = useCallback(async(text,base)=>{
    ...
    setTopicDone(false); // user is continuing — reset conclusion state
    ...
    try{
      ...
      // Detecta sinais de conclusão do Diretor para oferecer retorno ao menu
      const conclusionSignals = [
        "podemos seguir para outro assunto",
        "deseja trabalhar outro tema",
        "próximo assunto",
        "o que mais posso trabalhar",
        "posso te ajudar com mais alguma coisa",
        "quer explorar outro ponto",
        "ficou alguma dúvida",
        "está satisfeito com",
        "quando quiser retomar",
      ];
      const replyLower = reply.toLowerCase();
      if(conclusionSignals.some(s => replyLower.includes(s))){
        setTopicDone(true);
      }
  ...
  const OB=[
    { q:"Conecte sua planilha de shows.", h:"Cole a URL do Web App do Apps Script e o token. O Diretor consulta sua planilha em tempo real. Pode pular e conectar depois pelo ícone 🔗.", valid:()=>cfgForm.url.trim().length>0&&cfgForm.token.trim().length>0,
      skip:true,
      body:()=>(<>
        <div className="field"><label className="fl">URL do Web App (Apps Script)</label><input className="fi" value={cfgForm.url} placeholder="cole aqui a URL do Web App do Apps Script" onChange={e=>setCfgForm(f=>({...f,url:e.target.value}))}/></div>
        <div className="field"><label className="fl">Token secreto</label><input className="fi" value={cfgForm.token} placeholder="mesmo valor de SECRET_TOKEN definido no Code.gs" onChange={e=>setCfgForm(f=>({...f,token:e.target.value}))}/></div>
      </>)
    },
    { q:"Como você se apresenta ao mercado?", h:"Nome artístico, como você toca e onde atua.", valid:()=>form.nome.trim().length>0,
      body:()=>(<>
        <div className="field"><label className="fl">Nome artístico / marca</label><input className="fi" value={form.nome} placeholder="Ex: Maria Voz, Duo Canela, Trio Roots…" onChange={e=>setForm(f=>({...f,nome:e.target.value}))} /></div>
        ...
        <div className="field"><label className="fl">Cidade / região</label><input className="fi" value={form.regiao} placeholder="Ex: São Paulo, SP — Zona Sul" onChange={e=>setForm(f=>({...f,regiao:e.target.value}))} /></div>
      </>)
    },
    { q:"Seus gêneros e referências.", h:"Selecione o que você trabalha e informe suas influências.", valid:()=>form.generos.length>0,
      body:()=>(<>
        ...
        <div className="field" style={{marginTop:16}}><label className="fl">Referências e influências</label><textarea className="fta" value={form.referencias} placeholder="Ex: artistas que influenciam seu estilo…" onChange={e=>setForm(f=>({...f,referencias:e.target.value}))} /></div>
      </>)
    },
    { q:"Seu mercado atual.", h:"Onde você toca e quanto cobra.", valid:()=>form.tipos_evento.length>0,
      body:()=>(<>
        ...
        <div className="field" style={{marginTop:16}}><label className="fl">Cachê atual por show</label><input className="fi" value={form.cache_atual} placeholder="Ex: R$ 600 por show, R$ 1.200 aos fins de semana" onChange={e=>setForm(f=>({...f,cache_atual:e.target.value}))} /></div>
      </>)
    },
    { q:"Onde você quer chegar.", h:"Objetivos e o que te diferencia.", valid:()=>form.objetivos.length>0,
      body:()=>(<>
        ...
        <div className="field" style={{marginTop:16}}><label className="fl">Seu diferencial</label><textarea className="fta" value={form.diferencial} placeholder="Ex: versatilidade de gêneros, presença de palco, voz marcante…" onChange={e=>setForm(f=>({...f,diferencial:e.target.value}))} /></div>
      </>)
    }
  ];

  const step=OB[obStep]; const isLast=obStep===OB.length-1;
  // showQA: true only on fresh start OR after user explicitly returns to menu
  const hasUserMessages = messages.filter(m=>m.role==="user").length > 0;
  const showQA = !hasUserMessages && !loading;
    ...
    <div id="dm-root"><style>{CSS}</style>
      ...
      {showQA?(
        ...
        <div className="chat-area">
          ...
          {topicDone&&!loading&&(
            <div className="topic-done-bar">
              <span>Deseja trabalhar outro assunto?</span>
              <div className="topic-done-btns">
                <button className="topic-btn-continue" onClick={()=>setTopicDone(false)}>Continuar aqui</button>
                <button className="topic-btn-menu" onClick={()=>{
                  setTopicDone(false);
                  // Clear only the visible messages to show QA grid
                  // but keep them in memory ref so Director retains context
                  setMessages([]);
                  store.set(STORAGE_MSGS, JSON.stringify([]));
                }}>Nova ação ↗</button>
              </div>
