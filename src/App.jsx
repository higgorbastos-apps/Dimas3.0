...
export default function App() {
  ...
  const saveProfile = useCallback((p, isFirst) => {
    setProfile(p); profRef.current = p; setForm(p);
    store.set(STORAGE_PROFILE, JSON.stringify(p));
    setShowModal(false);
    isFirstRef.current = isFirst;
    setMessages([]); // garante mensagens zeradas
    setView("main");
  }, []);

  // Dispara a apresentação inicial DEPOIS que view e messages estão estáveis
  const introSentRef = useRef(false);
  useEffect(() => {
    if (view === "main" && isFirstRef.current && !introSentRef.current && profRef.current) {
      introSentRef.current = true;
      isFirstRef.current = false;
      // Pequeno delay para o React terminar de renderizar
      setTimeout(() => {
        const intro = "Apresentação inicial: acabei de receber seu briefing completo. Apresente-se profissionalmente e faça uma análise honesta do meu perfil — potencial real, o que me limita agora e a primeira ação concreta desta semana.";
        const userMsg = { role: "user", content: intro };
        setMessages([userMsg]);
        setLoading(true);
        callPuter([userMsg], profRef.current)
          .then(reply => {
            setMessages([userMsg, { role: "assistant", content: reply }]);
            store.set(STORAGE_MSGS, JSON.stringify([userMsg, { role: "assistant", content: reply }]));
          })
          .catch(e => {
            const msg = e.message === "timeout_ia"
              ? "⏱ A resposta demorou demais. Tente novamente."
              : `⚠️ ${e.message || "Erro. Tente novamente."}`;
            setMessages([userMsg, { role: "assistant", content: msg, isError: true }]);
          })
